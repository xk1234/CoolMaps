...
public class CityMarker extends CommonMarker {
	...
	public void showTitle(PGraphics pg, float x, float y)
	{
		...
		float tw = Math.max(pg.textWidth(name), pg.textWidth(pop));
		if (x + tw > 650) {
			pg.rect(x - tw, y-TRI_SIZE-39, tw + 6, 39);
			pg.fill(0, 0, 0);
			pg.text(name, x+3-tw, y-TRI_SIZE-33);
			pg.text(pop, x+3-tw, y - TRI_SIZE -18);
		} else {
			pg.rect(x, y-TRI_SIZE-39, tw + 6, 39);
			pg.fill(0, 0, 0);
